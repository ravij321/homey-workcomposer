...
app.get('/api/devices', requireAuth, async (_req, res) => {
  try { const result=await query(`SELECT d.id,d.external_id,d.serial_number,d.name,d.os,d.os_version,d.status,d.mdm_source,d.last_seen_at,u.name AS user_name FROM devices d LEFT JOIN users u ON u.id=d.user_id ORDER BY d.updated_at DESC`); res.json({devices:result.rows}); }
  catch { res.status(503).json({error:'Database unavailable'}); }
});
