...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, service: 'homey-work-insights-api' });
});
...
app.get('/api/dashboard', (_req, res) => {
  res.json({
    ...
    departments: [
      { name: 'Technology', hours: 126 },
      { name: 'Operations', hours: 82 },
      { name: 'Sales', hours: 51 },
      { name: 'Management', hours: 25 },
    ],
    applications: [
      { name: 'VS Code', minutes: 4820 },
      { name: 'Google Chrome', minutes: 4310 },
      { name: 'Slack', minutes: 2940 },
      { name: 'Terminal', minutes: 2180 },
    ],
  });
});
